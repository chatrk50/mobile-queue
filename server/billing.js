...
export async function renewDue() {
  ...
  const due = db.prepare(`SELECT id, slug, name, omise_customer_id, plan_name, plan_interval, plan_until, owner_email FROM tenants
  ...
  for (const t of due) {
    try {
      ...
      charged++;
      if (t.owner_email) receipts.push({ email: t.owner_email, name: t.name, slug: t.slug, plan: t.plan_name, interval: t.plan_interval, planUntil: until, amount });
    } catch (e) {
      failed++;
      ...
      if (!t.plan_until || t.plan_until < graceCut) {
        ...
        if (t.owner_email) downgrades.push({ email: t.owner_email, name: t.name, slug: t.slug, prevPlan: t.plan_name });
      }
    }
  }
