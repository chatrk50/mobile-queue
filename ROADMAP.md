...
- **Customer self-ordering + pay-at-counter — ✅ BUILT:** customers order in the LIFF (menu + topping popup) → get a queue number → **pay at the counter**. Orders carry `source` + `payment_status`; cashier sees PAID/UNPAID + LINE tag, with **Paid** and **Cancel** buttons. Routes: `POST /api/zones/:id/order`, `/tickets/:id/paid`, `/tickets/:id/void`. (Build-vs-adopt LINE MINI Eats analysis → **build into our LIFF**; see [docs/line-self-order-research.md](docs/line-self-order-research.md).)
- **Phase 4 — optional next:** PromptPay deposit/online prepay (LINE Pay or PromptPay QR); advance booking; multi-language LIFF; revisit a *Certified LINE MINI App* only if discovery-in-LINE or online prepay is wanted.
