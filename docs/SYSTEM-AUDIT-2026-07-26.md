...
## 5) ลำดับที่แนะนำ (แต่ละข้อ = 1 รอบสั้นๆ มี e2e กำกับ)

1. **ราคา order ฝั่ง server** (1.1) — ก่อนเปิดรับลูกค้า LINE วงกว้าง
2. **Transaction ห่อคู่เผาสิทธิ์+ส่วนลด** (1.6 เฉพาะ 4 จุดคูปอง/แต้ม) + guard สถานะ void (1.8)
3. **Index ชุดแรก** (1.7 — 5 บรรทัด additive, เสี่ยงต่ำ ได้เร็ว)
4. **ปิดรู PII เบอร์โทร** (1.2) + error middleware (1.5)
5. **Retention job** (push_log / slips / sale_events)
6. ก่อน merge สาย saas: **scope ทุกคำสั่งกวาด** (1.9) + branch scoping รวม (หนี้เดิม)
